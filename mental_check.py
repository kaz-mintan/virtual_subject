...
def out_kitekan_correct(data_num):
  factor = np.loadtxt("./virtual_data/"+dir_name+"/factor.csv",delimiter=",")
  #factor = np.loadtxt("./virtual_data/"+dir_name+"/factor_"+str(data_num+1)+".csv",delimiter=",")
  kibun = np.loadtxt("./virtual_data/"+dir_name+"/output/estimated_phi_"+str(data_num+1)+".csv",delimiter=",")
  if kibun.ndim == 0:
    length = 1
...
elif mode == 'mental_check':
  for test_num in range(1,N_RANGE):
    kibun = np.loadtxt("./virtual_data/"+dir_name+"/output/estimated_phi_"+str(test_num+1)+".csv",delimiter=",")
    if test_num%5==0:
      plt.plot(kibun,label='estimated_'+str(test_num),color = cm.hsv(test_num/float(N_RANGE+20.0)))
  mental = np.loadtxt("./virtual_data/"+dir_name+"/mental.csv",delimiter=",")
  #plt.plot(mental,label='answer',linestyle='dashed')
  plt.legend()
  plt.show()
    #for emo_num in range(EMO_NUM):
