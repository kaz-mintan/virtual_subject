...
factor = np.loadtxt("./virtual_data/"+dir_name+"/factor.csv",delimiter=",")[:N_RANGE,:]

def out_kitekan_correct(data_num):
  kibun = np.loadtxt("./virtual_data/"+dir_name+"/mental.csv",delimiter=",")[:N_RANGE]
...
for test_num in range(1,N_RANGE):
  ...
  signal = np.loadtxt("./virtual_data/"+dir_name+"/face.csv",delimiter=",")[:N_RANGE,:]
  #signal = np.loadtxt("./virtual_data/"+dir_name+"/face_"+str(N_RANGE)+".csv",delimiter=",")
  ...
  weight[0,:]= np.loadtxt("./virtual_data/"+dir_name+"/output/hap_weight_"+str(test_num+1)+".csv")
  weight[1,:]= np.loadtxt("./virtual_data/"+dir_name+"/output/sup_weight_"+str(test_num+1)+".csv")
  weight[2,:]= np.loadtxt("./virtual_data/"+dir_name+"/output/ang_weight_"+str(test_num+1)+".csv")
  weight[3,:]= np.loadtxt("./virtual_data/"+dir_name+"/output/sad_weight_"+str(test_num+1)+".csv")
  ...
  #if test_num%5==0:
    #plt.scatter(factor[:,TARGET_FACTOR],phi_signal[TARGET_FACE]-signal[:,TARGET_FACE],label="phi out"+str(test_num),color=cm.hsv(test_num/float(N_RANGE+20.0)),linewidth=0.5,linestyle='dashed')
  plt.plot(factor[:,TARGET_FACTOR],phi_signal[TARGET_FACE],label="phi out"+str(test_num),color=cm.hsv(test_num/float(N_RANGE+20.0)),linewidth=0.5,linestyle='dashed')
