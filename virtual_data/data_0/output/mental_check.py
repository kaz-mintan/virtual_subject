...
def out_kitekan_correct(data_num):
  ...
  kibun = np.loadtxt("./estimated_phi_"+str(data_num+1)+".csv",delimiter=",")
  if kibun.ndim == 0:
    length = 1
...
#standard
  ...
  for test_num in range(1,N_RANGE):
    kibun = np.loadtxt("./estimated_phi_"+str(test_num+1)+".csv",delimiter=",")
    if test_num%5==0:
      plt.plot(kibun,label='estimated_'+str(test_num),color = cm.hsv(test_num/float(N_RANGE+20.0)))
  mental = np.loadtxt("../mental.csv",delimiter=",")
